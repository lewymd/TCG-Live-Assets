...
//
///////////////////////////////////////////
Shader "TPCi/Boards/S_Water_BeachFoam_Reduced" {
Properties {
...
void main()
{
    u_xlat0.xy = vs_TEXCOORD0.zw + vec2(_FoamTendrilsOffset.x, _FoamTendrilsOffset.y);
    u_xlat0.xy = vs_TEXCOORD1.xy * _FoamTendrilsTiling.xy + u_xlat0.xy;
    u_xlat0.xy = _Time.yy * vec2(0.0, 0.0250000004) + u_xlat0.xy;
    u_xlat16_0 = texture(_WaterPatternBlur, u_xlat0.xy).y;
    u_xlat2.xy = vs_TEXCOORD0.zw + vec2(_FoamEdgeOffset.x, _FoamEdgeOffset.y);
    u_xlat2.xy = vs_TEXCOORD1.xy * _FoamEdgeTiling.xy + u_xlat2.xy;
    u_xlat1.xy = vs_TEXCOORD1.xy * _DistortionTiling.xy;
    u_xlat1.xy = _Time.yy * vec2(_DistortionPanner.x, _DistortionPanner.y) + u_xlat1.xy;
    u_xlat16_1.xy = texture(_DistortionTexture, u_xlat1.xy).xy;
    u_xlat2.xy = vec2(_DistortionAmount) * u_xlat16_1.xy + u_xlat2.xy;
    u_xlat16_2.xy = texture(_FoamRGBMask, u_xlat2.xy).xz;
    u_xlat4 = u_xlat16_2.y + u_xlat16_2.x;
#ifdef UNITY_ADRENO_ES3
    u_xlat4 = min(max(u_xlat4, 0.0), 1.0);
#else
    u_xlat4 = clamp(u_xlat4, 0.0, 1.0);
#endif
    u_xlat0.x = u_xlat16_0 * u_xlat4 + u_xlat16_2.x;
#ifdef UNITY_ADRENO_ES3
    u_xlat0.x = min(max(u_xlat0.x, 0.0), 1.0);
#else
    u_xlat0.x = clamp(u_xlat0.x, 0.0, 1.0);
#endif
    u_xlat2.xy = vs_TEXCOORD0.zw + vec2(_FoamEdgeOffset2.x, _FoamEdgeOffset2.y);
    u_xlat2.xy = vs_TEXCOORD1.xy * _FoamEdgeTiling2.xy + u_xlat2.xy;
    u_xlat2.xy = vec2(_DistortionAmount) * u_xlat16_1.xy + u_xlat2.xy;
    u_xlat16_2.xy = texture(_FoamRGBMask, u_xlat2.xy).xz;
    u_xlat4 = u_xlat16_2.y + u_xlat16_2.x;
#ifdef UNITY_ADRENO_ES3
    ...
    u_xlat4 = clamp(u_xlat4, 0.0, 1.0);
#endif
    u_xlat1.xy = vs_TEXCOORD0.zw + _FoamTendrilsOffset2.xy;
    u_xlat1.xy = vs_TEXCOORD1.xy * vec2(_FoamTendrilsTiling2.x, _FoamTendrilsTiling2.y) + u_xlat1.xy;
    u_xlat1.xy = _Time.yy * vec2(0.0, 0.0149999997) + u_xlat1.xy;
    u_xlat16_6 = texture(_WaterPatternBlur, u_xlat1.xy).y;
    u_xlat2.x = u_xlat4 * u_xlat16_6 + u_xlat16_2.x;
#ifdef UNITY_ADRENO_ES3
    u_xlat2.x = min(max(u_xlat2.x, 0.0), 1.0);
#else
    u_xlat2.x = clamp(u_xlat2.x, 0.0, 1.0);
#endif
    u_xlat1.xyz = u_xlat2.xxx * _FoamEdgeColorBottom.xyz;
    u_xlat2.xyz = (-_FoamEdgeColorBottom.xyz) * u_xlat2.xxx + _FoamEdgeColorTop.xyz;
...
#endif
    u_xlat6 = vs_TEXCOORD1.y + 0.419999987;
    u_xlat6 = (-u_xlat6) + 1.0;
    u_xlat0.w = u_xlat6 * 7.05000019;
#ifdef UNITY_ADRENO_ES3
    u_xlat0.w = min(max(u_xlat0.w, 0.0), 1.0);
#else
    u_xlat0.w = clamp(u_xlat0.w, 0.0, 1.0);
#endif
    u_xlat0 = u_xlat0 * vec4(1.5, 1.5, 1.5, 1.70000005);
